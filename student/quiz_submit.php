...
$s=db()->prepare("SELECT id,correct_option FROM quiz_questions WHERE quiz_id=? AND activity_type='lesson_quiz' AND status='active'");$s->bind_param('i',$qid);$s->execute();$qs=$s->get_result()->fetch_all(MYSQLI_ASSOC);
