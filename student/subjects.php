...
$s=db()->prepare("SELECT s.*,(SELECT COUNT(*) FROM lessons l WHERE l.subject_id=s.id AND l.grade_id=? AND l.status='active' AND l.content_source='textbook' AND (l.medium='All' OR l.medium=?)) lesson_count,(SELECT COUNT(*) FROM lessons l WHERE l.subject_id=s.id AND l.grade_id=? AND l.status='active' AND l.content_source='textbook' AND (l.medium='All' OR l.medium=?) AND (EXISTS(SELECT 1 FROM lesson_progress lp WHERE lp.user_id=? AND lp.lesson_id=l.id AND lp.completed_at IS NOT NULL) OR EXISTS(SELECT 1 FROM student_points sp WHERE sp.user_id=? AND sp.lesson_id=l.id AND sp.activity_type='lesson_complete'))) completed_count FROM subjects s WHERE s.grade_id=? AND s.status='active' HAVING lesson_count>0 ORDER BY s.name_en");
if(!$s){flash('error','Subjects could not be loaded. Please try again.');redirect('student/dashboard.php');}$s->bind_param('isisiii',$grade,$medium,$grade,$medium,$uid,$uid,$grade);$s->execute();$res=$s->get_result();
$copy=[
