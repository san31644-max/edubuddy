...
try{
 ...
 $_SESSION['referral_promoter']=query_one("SELECT id,full_name,email,phone,referral_code,status,created_at FROM referral_promoters WHERE id=?",'i',[$promoterId]);$_SERVER['SCRIPT_NAME']='/educhat/referral/dashboard.php';$_SERVER['HTTP_HOST']='localhost';ob_start();include __DIR__.'/../referral/dashboard.php';$portalHtml=ob_get_clean();referral_assert(str_contains($portalHtml,'Referral Test Student')&&str_contains($portalHtml,'Paid Rs. 280'),'Promoter dashboard renders the referred student and verified purchase');unset($_SESSION['referral_promoter']);
