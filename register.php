...
require_guest();
$errors=[];$notice='';
$v=['full_name'=>'','school_name'=>'','district'=>'','phone'=>'','grade_id'=>'','medium'=>'English'];
foreach($v as $k=>$default)$v[$k]=trim((string)($_POST[$k]??$default));
$action=(string)($_POST['action']??'');
if($_SERVER['REQUEST_METHOD']==='POST'){
    verify_csrf();$phone=normalize_sri_lankan_phone($v['phone']);
    ...
    if($action==='send_otp'&&!$errors){
        ...
        if(!$errors){$language=medium_language($v['medium']);$newId=(int)(query_one('SELECT COALESCE(MAX(id),0)+1 next_id FROM users')['next_id']??1);$username='u'.substr($phone,-9);$suffix=0;while(query_one('SELECT id FROM users WHERE username=?','s',[$username]))$username='u'.substr($phone,-7).(++$suffix);$hash=password_hash($p,PASSWORD_DEFAULT);$gradeId=(int)$v['grade_id'];$s=db()->prepare("INSERT INTO users(id,full_name,username,email,phone,phone_verified_at,school_name,district,password_hash,grade_id,medium,preferred_language,profile_image,subscription_expires_at,status,created_at,updated_at) VALUES(?,?,?,NULL,?,NOW(),?,?,?,?,?,?,NULL,NULL,'active',NOW(),NOW())");if(!$s){error_log('Registration prepare failed: '.db()->error);$errors[]='Registration is temporarily unavailable.';}else{$s->bind_param('issssssiss',$newId,$v['full_name'],$username,$phone,$v['school_name'],$v['district'],$hash,$gradeId,$v['medium'],$language);if($s->execute()){unset($_SESSION['phone_otp']);$_SESSION['onboarding_user_id']=$newId;flash('success','Account created. Log in with your phone number.');redirect('login.php');}error_log('Registration execute failed '.$s->errno.': '.$s->error);$errors[]='Account could not be created. Please retry.';}}
    }
}
