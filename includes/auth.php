...
function refresh_user():void{if(!user())return;$u=query_one('SELECT id,full_name,username,email,school_name,grade_id,medium,preferred_language,profile_image,subscription_expires_at,status FROM users WHERE id=?','i',[user()['id']]);if($u){$u['preferred_language']=medium_language((string)$u['medium']);$_SESSION['user']=$u;$_SESSION['lang']=$u['preferred_language'];}}
