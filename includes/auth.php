...
require_once __DIR__.'/db.php';require_once __DIR__.'/helpers.php';require_once __DIR__.'/gamification.php';require_once __DIR__.'/activity.php';
...
function refresh_user():void{if(!user())return;$u=query_one('SELECT id,full_name,username,email,phone,phone_verified_at,school_name,district,grade_id,medium,preferred_language,profile_image,subscription_expires_at,status FROM users WHERE id=?','i',[user()['id']]);if($u){$u['preferred_language']=medium_language((string)$u['medium']);$_SESSION['user']=$u;$_SESSION['lang']=$u['preferred_language'];}}
